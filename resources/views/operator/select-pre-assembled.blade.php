    ...
    <div class="max-w-6xl h-fit mx-auto my-0 border border-gray-900 p-4 rounded shadow mt-9">
        <form wire:submit.prevent="submit">
            ...
            <table class="w-full h-max-content border border-gray-900 text-sm">
                ...
                <tbody class="block max-h-[45vh] overflow-y-auto w-full">
                    @foreach ($preassembleds as $p)
                    <tr class="table w-full table-fixed text-white">
                        <td class="border border-gray-900 w-2/4 p-5 ">{{$p->padre_description}}</td>
                        <td class="border border-gray-900 w-1/4 px-2 py-1">{{$p->code}}</td>
                        <td class="border border-gray-900 w-1/4 p-0">
                            <div class="flex justify-center items-center h-full px-3">
                                <a href="{{route('lotto.create', $p->id)}}" class="bg-azure-400 text-black font-bold rounded hover:bg-azure-700 transition p-3 w-full h-full text-center">
                                    Seleziona
                                </a>
                            </div>
                        </td>
